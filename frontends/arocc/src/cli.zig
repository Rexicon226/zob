...
pub fn ArgumentInfoGroup(comptime S: type) type {
    ...
    var field_names: [s_info.fields.len][]const u8 = undefined;
    var field_types: [s_info.fields.len]type = undefined;
    const field_attrs: [s_info.fields.len]Type.StructField.Attributes = @splat(.{});
    for (
        &field_names,
        &field_types,
        s_info.fields,
    ) |*new_s_field_name, *new_s_field_type, s_field| {
        ...
        new_s_field_name.* = s_field.name;
        new_s_field_type.* = s_field.type;
    }
...
fn CmdHelper(
    ...
    = mappings_and_init: {
        var arg_enum_field_names: []const []const u8 = &.{};
        var arg_enum_field_tags: []const ArgEnumInt = &.{};
        ...
        const computeArgFieldInfo = struct {
            ...
            fn computeArgFieldInfo(
                ...
                //
                comptime arg_enum_field_names_ptr: *[]const []const u8,
                comptime arg_enum_field_tags_ptr: *[]const ArgEnumInt,
                ...
                switch (arg_info.kind) {
                    .named => {
                        const arg_enum_field_idx = arg_enum_field_names_ptr.len;
                        arg_enum_field_names_ptr.* = arg_enum_field_names_ptr.* ++ .{arg_enum_field_name};
                        arg_enum_field_tags_ptr.* = arg_enum_field_tags_ptr.* ++ .{arg_enum_field_idx};
    ...
    = alias_info: {
        ...
        const UsedAliasesBits = @Int(.unsigned, MAX_ALIAS_TABLE_LEN);
    ...
    return struct {
        ...
        fn parseInner(
            ...
            const parse_result: union(enum) {
                ...
                parse_arg: {
                    ...
                    if (is_help) {
                        try writeHelp(command_chain, terminal);
                        break .help;
                    }
                ...
                if (positional_count != positional_set.len) {
                    ...
                    switch (positional_count) {
                        inline 0...positional_set.len - 1 => |positional_i| {
                            ...
                            const args_iter_index_guard = arg_iter_index;
                            try parseArgValueMaybeScanIntoResult(
                                allocator,
                                s_field_idx,
                                &result,
                            ...
                            std.debug.assert(arg_iter_index == args_iter_index_guard); // sanity check
            ...
            std.debug.assert(!args_iter.skip() or parse_result == .help);
        ...
        fn writeHelp(
            ...
            command_chain: anytype,
            terminal: std.Io.Terminal,
        ) (std.Io.Writer.Error || std.Io.Terminal.SetColorError)!void {
            try terminal.setColor(.yellow);
            try terminal.writer.writeAll("USAGE:\n");
            try terminal.writer.splatByteAll(' ', 2);

            try terminal.setColor(.green);
            try terminal.writer.writeAll(command_chain[0]);
            try terminal.writer.writeByte(' ');
            for (command_chain[1..]) |subcmd_str| {
                try terminal.writer.writeAll(subcmd_str);
                try terminal.writer.writeByte(' ');
            }
            ...
            inline for (positional_set) |s_field_idx| {
                ...
                try terminal.writer.writeAll("[" ++ arg_name ++ "] ");
            }

            try terminal.writer.writeAll("[OPTIONS]");

            try terminal.setColor(.reset);
            try terminal.writer.splatByteAll('\n', 2);
            try terminal.writer.writeAll(cmd_info.help.short);

            if (cmd_info.help.long) |long_help| {
                try terminal.writer.splatByteAll('\n', 2);
                try terminal.writer.writeAll(long_help);
            }
            ...
            if (maybe_subcmd_list) |subcmd_list| {
                ...
                try terminal.setColor(.yellow);
                try terminal.writer.writeAll("COMMANDS:");
                ...
                for (subcmd_list) |name_help_pair| {
                    ...
                    try terminal.writer.writeAll(subcmd_name);
                    const padding = name_base_width - subcmd_name.len + 3;
                    try terminal.setColor(.reset);
                    try terminal.writer.splatByteAll(' ', padding);

                    const indent = 2 + name_base_width + 3;
                    try writeIndentedText(terminal.writer, indent, subcmd_short_help);
                }
            ...
            const default_value_base_width: ?u64 = try writeArgumentsHelp(
                .{ .writer = &discard.writer, .mode = .no_color },
                0,
                null,
            );
        ...
        fn writeArgumentsHelp(
            ...
            inline for (positional_set, 0..) |s_field_idx, i| {
                ...
                try terminal.setColor(.green);
                if (multiline_help) try terminal.writer.writeByte('\n');
                try terminal.writer.writeByte('\n');
                try terminal.writer.splatByteAll(' ', 2);

                // write argument alias & name
                var cw: std.Io.Writer.Discarding = .init(&.{});
                for ([_]*std.Io.Writer{ &cw.writer, terminal.writer }) |w| {
                ...
                // write padding
                try terminal.writer.splatByteAll(' ', 1 + padding: {
                ...
                try writeIndentedText(terminal.writer, indent, arg_info.help);
                if (multiline_help and i != positional_set.len - 1) try terminal.writer.writeByte('\n');
            }

            if (positional_set.len != 0) try terminal.writer.writeByte('\n');
            ...
            try terminal.writer.writeAll(help_argument_alias_name);
            const padding1 =
            ...
            try terminal.setColor(.reset);
            try terminal.writer.splatByteAll(' ', padding1 + padding2);

            try terminal.writer.writeAll("Prints help information\n");
...
fn computeCmdAndArgBasicInfo(
    ...
    var field_names: [s_info.fields.len][]const u8 = undefined;
    var field_types: [s_info.fields.len]type = undefined;
    var field_attrs: [s_info.fields.len]Type.StructField.Attributes = undefined;
    ...
    for (
        ...
        s_info.fields,
        0..,
    ) |*new_s_field_name, *new_s_field_type, *new_s_field_attr, s_field, s_field_i| {
        ...
        new_s_field_name.* = s_field.name;
        new_s_field_type.* = FieldType;
        new_s_field_attr.* = .{ .@"align" = @alignOf(FieldType) };
    }
...
fn UnionArgDescSubMap(comptime U: type) type {
    ...
    var field_names: [sub_u_info.fields.len][]const u8 = undefined;
    var field_types: [sub_u_info.fields.len]type = undefined;
    var field_attrs: [sub_u_info.fields.len]Type.StructField.Attributes = undefined;
    ...
    for (
        ...
        &field_attrs,
        sub_u_info.fields,
    ) |*new_s_field_name, *new_s_field_type, *new_s_field_attr, u_field| {
        new_s_field_name.* = u_field.name;
        new_s_field_type.* = CommandInfo(u_field.type);
        new_s_field_attr.* = .{ .@"align" = @alignOf(CommandInfo(u_field.type)) };
    }
...
inline fn renderArgumentDefaultValue(
    ...
    const value, const fmt_str = if (T == []const u8)
        .{ std.zig.fmtString(default_value), "" }
